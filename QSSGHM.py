...
T_input = df['Dry bulb temp (C)'].values
P_input = df['Pressure (millibar)'].values / 10  # in kPa
v_input = df['Wind speed (m/s)'].values
Ig_input = df['Global irradiance - GHI (W/m2)'].values
It_e1 = df['POA front total irradiance after shading and soiling (W/m2)-e1'].values
It_e2 = df['POA front total irradiance after shading and soiling (W/m2)-e2'].values
It_w1 = df['POA front total irradiance after shading and soiling (W/m2)-w1'].values
It_w2 = df['POA front total irradiance after shading and soiling (W/m2)-w2'].values
It_n = df['POA front total irradiance after shading and soiling (W/m2)-n'].values
It_s = df['POA front total irradiance after shading and soiling (W/m2)-s'].values
hr_no = df['Hours since 00:00 Jan 1'].values
w_d = df['Wind direction (deg)'].values
sun_rs = df['average sunrise hour'].values
sun_st = df['average sunset hour'].values

sun_rs = sun_rs[~np.isnan(sun_rs)]
sun_st = sun_st[~np.isnan(sun_st)]

T_amb = np.zeros((365, 24)) # has 365 rows and 24 columns
...
for i in range(365):
    T_amb[i] = T_input[i*24:24*(i+1)].transpose()
    P_amb[i] = P_input[i*24:24*(i+1)].transpose() # kPa
    v_amb[i] = v_input[i*24:24*(i+1)].transpose() # m/s
    w_dir[i] = w_d[i*24:24*(i+1)].transpose()  # degree
    I_G[i] = Ig_input[i*24:24*(i+1)].transpose()  # W/m2
    I_T_e1[i] = It_e1[i*24:24*(i+1)].transpose()  # W/m2
    I_T_e2[i] = It_e2[i*24:24*(i+1)].transpose()  # W/m2
    I_T_w1[i] = It_w1[i*24:24*(i+1)].transpose()  # W/m2
    I_T_w2[i] = It_w2[i*24:24*(i+1)].transpose()  # W/m2
    I_T_n[i] = It_n[i*24:24*(i+1)].transpose()  # W/m2
    I_T_s[i] = It_s[i*24:24*(i+1)].transpose()  # W/m2
    hr_num[i] = hr_no[i*24:24*(i+1)].transpose()  # hour number
...
for i in range(365):
    ...
    for j in range(len(loc_day)):
        if loc_day[j] > (24 * i) and loc_day[j] < (24 * (i + 1)):
            sf[i, n] = loc_day[j] - 24 * i # location of daylight hours separately for each day
            n = n + 1

n_day = np.zeros((365, 1)) # do we want an array or a matrix?
...
for i in range(365):
    ...
    loc_end_sl[i][0] = sun_st[i] + n_exc_light[i] # end point of supplementary lighting
...
for i in range(365):
    for j in range(24):
        ...
        R_s[i][j]=200*(1+1/math.exp(0.05*(I_G[i][j]*Trans_GL-50)))
